...
class Uploader {
	...
	public function upload($file, array $options = array()) {
		...
		if (!$options['multiple']) {
			if (!$this->_enabled) {
				echo "Modo de upload multiple no habilitado";
				return false;
			} else {
				$this->checkDirectory();
			}
		...
		if ($this->_current) {
			...
			echo "No existe archivo actual";
			return false;
		}

		// Validate everything
		if (!$this->_validates()) {
			echo "No se pudo pasar la validacion";
			return false;
		}
		...
		// Uploaded via POST
			...
			if (move_uploaded_file($current['tmp_name'], $dest)) {
				...
				echo "No se pudo copiar el archivo a su destino por ninguno de los 2 metodos";
				return false;
			}
		}
	...
	protected function _validates($import = false) {
		...
		if (!$import && !isset($current['stream'])) {
			if (($current['error'] > 0) || !is_uploaded_file($current['tmp_name']) || !is_file($current['tmp_name'])) {
				echo "Hubo error, no se encontró el archivo o no es un archivo lo subido";
				return false;
			}
			...
			if ($this->scanFile && $this->_loadExtension('clamav')) {
				...
				if (cl_scanfile($current['tmp_name'])) {
					echo "Error, el archivo tiene Virus!";
					return false;
				}
			}
		}
